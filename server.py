...
class FileServer:
    #INIT SERVER WITH INITIAL ATTRIBUTE
    ...
    #HANDLE CONNECTED CLIENT: CREATE CLIENT DICTIONARY AND START RECIEVE MESSAGE
...
########{client_address: {"hostname": hostname,"status": ?, "files": [{"local_name": local_name, "file_name": file_name}]}}###########
    ...
    def set_hostname(self, client_socket, client_address, hostname):
        with self.lock:
            ...
            if any(data["hostname"] == hostname for addr, data in self.clients.items() if addr != client_address):
